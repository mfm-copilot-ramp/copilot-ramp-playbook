...
- **GitHub Copilot harness agents** — the autonomous, agentic engine is **never** covered by a Microsoft 365
  ...
  Microsoft bills this harness **per task, by complexity** and publishes only credit **ranges** — **Light 100–300 ·
  Medium 300–500 · Heavy &gt;500 credits per task** — that bundle model tokens, tools, and the harness itself. A single
  interaction is a multi-step agentic task (reasoning + tools + files), so it costs materially more per turn than a
  standard topic, and there is **no per-action rate card** for this harness. See
  [Pick the engine](pick-the-engine.md); the [Credit Estimator](credit-estimator.md) models it with an editable
  **credits-per-task** anchor seeded from the complexity tier (biased to the high end of each published band) plus a
  one-time build &amp; test cost.
