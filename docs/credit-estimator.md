...
<div class="mode-panel em-hidden" id="panel-import">
  ...
  <div class="em-range" style="border-left:3px solid var(--md-primary-fg-color); padding:0.55rem 0.8rem; margin:0.7rem 0; background:var(--md-code-bg-color); border-radius:0 6px 6px 0;">
    <strong>Tip — let Copilot fill the sheet for you.</strong> You don't have to fill the <strong>Scenarios</strong> sheet by hand. Ask <strong>Microsoft 365 Copilot</strong> (Copilot Chat / Cowork, or Copilot in Excel) to populate it from a plain-English list of your agents — one row each — then re-upload. The <strong>example prompt below is copy-paste-ready</strong> — it already spells out every column, so there's nothing else to paste in. Hit <strong>Copy prompt</strong>, then just replace the <em>My agents</em> list at the bottom with your own and send it to Copilot; it uses the prefilled <strong>Examples</strong> sheet as the pattern, infers each column, and leaves unknowns blank. Example prompt:
    <pre id="qi-copilot-prompt" style="white-space:pre-wrap; font-size:0.76rem; margin:0.5rem 0 0.3rem; padding:0.5rem 0.65rem; background:var(--md-default-bg-color); border:1px solid var(--md-default-fg-color--lightest); border-radius:4px;">You're helping me fill in the 'Scenarios' sheet of this Copilot Credit Estimator workbook. Use the 'Examples' sheet as the pattern. Create one row per agent from my list, inferring each column from my descriptions; leave a cell blank if it isn't implied. Columns: Scenario name; Agent type (Interactive/Autonomous); Channel (chat/voice); Voice minutes / conversation; Knowledge grounding (none/docs/tenant graph); System actions (#); Back-end systems (#); Document processing? (yes/no); Draft / summarize content? (yes/no); Approval / automation flow? (yes/no); Human escalation? (yes/no); Users in scope; Interactions / user / month; Deployment (embedded/standalone); % with M365 Copilot license; Events / month; Generative steps / event; Description. My agents (one per line — what it does, who uses it, how often, channel, knowledge, actions):
1)
2)</pre>
